...
export function* handleRegister(action: PayloadAction<ModelAuthLogin>) {
    try {
        ...
        // const bouquet: string = yield select((state: RootState) => state.Bouquets.activeBouquet);
        const result: ModelAuthLoginResponse = yield ApiAuthRepo.register(action.payload)
        ...
        var message = ErrorHelper.getErrorMessage(error)
        console.log("handleRegister error message: ", message)
...
export function* watchRegister() {
    yield takeLatest(StoreActionAuth.register.type, handleRegister);
}
...
export function* watchAuth() {
    yield all([fork(watchAuthInit), fork(watchLogin), fork(watchRegister)]);
}